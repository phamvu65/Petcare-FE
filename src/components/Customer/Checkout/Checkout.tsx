...
const Checkout: React.FC = () => {
  ...
  // 🟢 6. XỬ LÝ ĐẶT HÀNG (Đã Fix Null ID)
  const handlePlaceOrder = async () => {
    ...
    // Kiểm tra ID sản phẩm trước khi gửi
    ...
    try {
      const payload = {
        ...
        note: note,
        // Fallback: Nếu productId null, dùng id
        items: isBuyNowMode 
            ? items.map(item => ({ 
      ...
      console.log("Order Payload:", payload); // Debug xem dữ liệu gửi đi

      const res = await api.post("/orders", payload);
      alert(`🎉 Đặt hàng thành công! Mã đơn: ${res.data.data.id}`);
      navigate("/profile"); 
    } catch (error: any) {
